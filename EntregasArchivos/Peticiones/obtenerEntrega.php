...
function realizarConsulta($queryCorrecto){
    ...
    if (mysqli_num_rows($comando) == 0) {
        ...
        while ($datos = mysqli_fetch_assoc($comando)) {  
            $Ruta = "Recibos/re". $datos["IdEntrega"] .".pdf"; //Se genera la ruta del archivo con el id consultado
            if (file_exists('../'.$Ruta)) //Se tiene que poner la ruta relativa del archivo apartir donde se encunetra este documento 
                $datos["Ruta"] = $Ruta; //Si el archivo existe se asigna la ruta al objeto que se va a mandar como respuesta
            else
                $datos["Ruta"] = ""; //Si no se manda vacio
            //var_dump($datos);
            $consulta["data"][] = $datos;
        } 
