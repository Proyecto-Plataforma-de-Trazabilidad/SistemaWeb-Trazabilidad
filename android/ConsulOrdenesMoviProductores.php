...
if($_SERVER['REQUEST_METHOD']=='POST'){ 
   ...
   $conn = new PDO('mysql:host='.$host_name.';dbname='.$database,$host_user,$host_password);
    ...
    switch($_POST['opcion']){
        ...
        case 'consulTQorden':
            $tq=$_POST['tq'];
            $query="SELECT O.IdOrden,D.Nombre AS Distribuidor, O.NumFactura,O.NumReceta FROM ordenproductos AS O INNER JOIN distribuidores AS D ON O.IdDistribuidor=D.IdDistribuidor INNER JOIN detalleorden AS DOR ON O.IdOrden=DOR.IdOrden WHERE DOR.IdTipoQuimico='2'";
            $resultado=$conn->prepare($query);
            $resultado->execute();
            $res = $resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
